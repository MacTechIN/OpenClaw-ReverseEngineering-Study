...
복잡해 보이지만 걱정하지 마십시오. 우리는 이 거대한 우주선을 **"단 5개의 버튼"** 으로 분해해서 하나씩 조립할 것입니다. (이것이 앞에서 설명드린 **추상화**를 의미하는 것 기억 나시죠?)
...
>
> **요약**: LLM은 **'지식의 창고'**이자 **'생각의 엔진'**입니다. 이 엔진에 '팔다리(Tool)'를 달아준 것이 바로 여러분이 만들고 있는 **에이전트**입니다.
